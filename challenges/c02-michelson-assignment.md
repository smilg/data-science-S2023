...
- My values are very close to, but not the same as Michelson’s. 2 km/s
  is the largest variation.

- The means for distinctness 1 and 2 are really close together, but the
  one for distinctness 3 is smaller/has a larger gap.

- The means increase with distinctness, but I don’t know if that
  actually tells me anything. If I understand the distinctness
  measurement correctly, it could mean that the speed is on the higher
  side relative to the data, since the “more accurate(?)” measurements
  were higher.
...
- Some sort of pre-processing before finding the means
...
- Is Michelson’s estimate of the error (his uncertainty) greater or less
  ...
  - Michelson’s estimate is less than the true error in magnitude.

- Make a quantitative comparison between Michelson’s uncertainty and his
  error.

  - Michelson’s uncertainty was less than his error.
...
- Similarities
  ...
  - The behavior of the means is similar - almost all of them fall
    inside the error lines.

  - Generally, it looks like the points for a given day are bunched
    together in both sets, though I would say the real data seems to
    exhibit this more strongly.

<!-- -->

- Differences

  - The real data has more means outside of the error lines in the
    earlier data, while the simulated data has means outside the error
    lines later in the data.

  - Both datasets have similar amounts of points that lie outside the
    error lines, but the real data has a few that are much further
    outside of the error lines than the furthest ones in the simulated
    data. This makes sense to me, since I’d imagine including such
    points in a model like this might just make a worse model (although
    I know nothing about models like this; that’s just a guess).
...
``` r
df_q2 %>% 
  ggplot(aes(x = Temp, color = Distinctness, group = Distinctness)) +
  geom_density()
```

![](c02-michelson-assignment_files/figure-gfm/q5-density-1.png)<!-- -->

**Observations**:

- Observations with a distinctness of 1 and 2 had a relatively even
  spread across temperatures, with 1 having a bit more towards lower
  temperatures.

- Distinctness 3 observations mostly occurred when the temperature was
  higher, with the peak between 75-80 degrees.

``` r
# have Date, Distinctness, Temp, VelocityVacuum (and Velocity)
df_q2 %>% 
  ggplot(aes(x = Temp, y = VelocityVacuum, color = Distinctness)) +
  geom_point()
```

![](c02-michelson-assignment_files/figure-gfm/q5-scatter-1.png)<!-- -->

**Observations**:

- There doesn’t appear to be much of any correlation between
  VelocityVacuum and Temperature.
- The means from q1 showed this earlier, but it looks like there may be
  a weak relationship between distinctness and velocity, though it is
  not clear.
- It does not appear that there are any factors in the data that explain
